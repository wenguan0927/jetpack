...
class Jetpack {
	 ...
	 *      'wordpress'                     => (int) Number of WordPress core updates available. // phpcs:ignore WordPress.WP.CapitalPDangit
	...
	public static function activate_new_modules( $redirect = false ) {
		...
		self::activate_default_modules( $jetpack_version, JETPACK__VERSION, $reactivate_modules );
	...
	function admin_init() {
		...
		if ( ! self::is_active() && ! self::is_development_mode() ) {
			...
			Jetpack_Client::_wp_remote_request(
				self::fix_url_for_bad_hosts( self::api_url( 'test' ) ),
				$args,
				true
			);
		}
...
<div id="message" class="jetpack-message jetpack-err">
	...
	public static function register() {
		...
		$response = Jetpack_Client::_wp_remote_request( self::fix_url_for_bad_hosts( self::api_url( 'register' ) ), $args, true );
	...
	function verify_xml_rpc_signature() {
		...
		if ( ! $signature ) {
			...
			return $signature;
		} elseif ( ! hash_equals( $signature, $_GET['signature'] ) ) {
			return false;
		}
